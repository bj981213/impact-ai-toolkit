...
if (catalog.items.filter((item) => "prompt" in item).length !== 21) errors.push("Exactly 21 tools must define prompt");
...
if (indexText.includes("提示：按下") || indexText.includes("可快速搜尋") || appText.includes('event.key === "/"')) errors.push("Removed search shortcut hint or behavior is still present");
...
if (publicText.includes("多封信件重點與待辦彙整")) errors.push("Public site contains the removed email batch tool");
for (const requiredNewTool of ["工作交接清單", "SOP 作業流程整理", "內部公告與通知草稿", "多份報表彙整與差異整理", "信件與表單待辦整理 Agent"]) {
