import { composeToolContent } from "./tool-content.js?v=2.9.0";
...
async function init() {
  try {
    const response = await fetch("data/catalog.json?v=2.9.0", { cache: "no-store" });
    ...
    <article class="need-card">
      ...
      <div class="need-links">
        ${stage.visibleIds.map((id) => {
          const item = findItem(id);
          return item ? `<a href="tools/${escapeAttr(item.id)}.html?v=2.9.0" data-open="${escapeAttr(item.id)}">${escapeHtml(item.title)}</a>` : "";
        }).join("")}
      </div>
    </article>
  `).join("");
  ...
  return `
    <article class="tool-card ${escapeAttr(item.toolbox)}-card">
      ...
      <div class="card-actions">
        ...
        <a href="tools/${escapeAttr(item.id)}.html?v=2.9.0" data-open="${escapeAttr(item.id)}">開啟工具</a>
      </div>
    </article>
  `;
}
...
function renderRecent() {
  ...
  $("#recentList").innerHTML = entries.length
    ? entries.map((entry) => `<li><a href="tools/${escapeAttr(entry.id)}.html?v=2.9.0" data-open="${escapeAttr(entry.id)}">${escapeHtml(entry.title)}</a><br><span>${escapeHtml(entry.kind)}・${formatDate(entry.visitedAt)}</span></li>`).join("")
